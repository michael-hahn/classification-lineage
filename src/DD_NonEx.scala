...
class DD_NonEx[T:ClassTag,K:ClassTag] {
  ...
  def test(inputRDD: RDD[T], testFunc: userTest[T], maxClusters: Int, totalClusters: Int, centroid_ref: Array[Cluster], lm: LogManager, fh: FileHandler): Boolean = {
    testFunc.usrTest(inputRDD, maxClusters, totalClusters, centroid_ref, lm, fh)
  }
  ...
  private def dd_helper(inputRDD: RDD[(T,K)],
    ...
    while (true) {
      ...
      val assertResult = test(rdd.map(x=>x._1), testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
      if (!assertResult) {
      ...
      breakable {
        for (i <- 0 until partitions) {
          val result = test(rddList(i).map(x => x._1), testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
          if (result) {
            rdd_failed = true
            next_rdd = rddList(i)
            next_partitions = 2
      ...
      if (!rdd_failed) {
        breakable{
          for (j <- 0 until partitions){
            ...
            val result = test(rddBar.map(x=>x._1), testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
            if (result){
              rddBar_failed = true
  ...
  def ddgen(inputRDD: RDD[(T,K)], testFunc: userTest[T], splitFunc: userSplit[T,K], maxClusters: Int, totalClusters: Int, centroid_ref: Array[Cluster], lm: LogManager, fh: FileHandler): RDD[(T,K)] = {
